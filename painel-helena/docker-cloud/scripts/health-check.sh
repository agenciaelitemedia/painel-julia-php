...
# Check services (apenas os locais)
SERVICES=(
    "rabbitmq"
    "frontend"
    "nginx"
...
echo -e "\n${GREEN}========================================${NC}"
echo -e "${GREEN}Status Geral: ${HEALTHY}/${TOTAL} serviços locais saudáveis${NC}"
echo -e "${GREEN}========================================${NC}"

# Load environment to check Supabase
if [ -f .env ]; then
    export $(cat .env | grep -v '^#' | xargs)
    
    echo -e "\n${BLUE}Verificando Supabase Cloud...${NC}"
    
    # Test Supabase connection
    SUPABASE_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -H "apikey: ${VITE_SUPABASE_ANON_KEY}" "${VITE_SUPABASE_URL}/rest/v1/")
    
    if [ "$SUPABASE_STATUS" == "200" ]; then
        echo -e "${GREEN}✅ Supabase Cloud: Conectado${NC}"
    else
        echo -e "${RED}❌ Supabase Cloud: Erro (HTTP ${SUPABASE_STATUS})${NC}"
        echo -e "${YELLOW}   Verifique suas credenciais e URL${NC}"
    fi
fi
...
if [ ${HEALTHY} -eq ${TOTAL} ]; then
    echo -e "\n${GREEN}🎉 Todos os serviços locais estão saudáveis!${NC}"
    exit 0
else
